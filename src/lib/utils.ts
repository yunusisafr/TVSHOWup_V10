...
/**
 * Check if the current route is an authentication page where ads should not be shown
 * @returns {boolean} True if the current route is an auth page
 */
export const isAuthPage = (): boolean => {
  ...
  return authPaths.some(authPath =>
    pathname === authPath ||
    pathname.includes(`/${authPath.slice(1)}`) ||
