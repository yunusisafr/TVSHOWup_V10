...
const ContentDetailPage: React.FC<ContentDetailPageProps> = ({ contentType }) => {
  const { lang, id: urlId, slug: urlSlug } = useParams<{ lang: string; id: string; slug?: string }>()
  const navigate = useNavigate()
  const { user } = useAuth()
  const { countryCode, languageCode } = useUserPreferences()
  const activeLanguage = lang || languageCode || 'en'
  ...
  const loadContent = async () => {
    try {
      ...
      if (!dbContent) {
        ...
        try {
          ...
          if (activeLanguage !== 'en') {
            const tmdbUrlLang = `https://api.themoviedb.org/3/${tmdbType}/${contentId}?api_key=${import.meta.env.VITE_TMDB_API_KEY}&language=${activeLanguage}&append_to_response=credits`
      ...
      if (shouldFixUrl && dbSlug) {
        const expectedPath = buildLanguagePath(`/${contentType}/${dbSlug}`, activeLanguage);
  ...
  const handleShare = async () => {
   ...
   const shareUrl = `${window.location.origin}${buildLanguagePath(contentPath, activeLanguage)}`
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {/* Ad Banner - Middle */}
            <div className="py-4">
              <AdBanner position="content_middle" />
            </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  {cast.map((person) => (
                    <Link
                      key={person.id}
                      to={buildLanguagePath(`/person/${createPersonSlug(person.id, person.name)}`, activeLanguage)}
          ...
          <div className="space-y-6">
            {/* Ad Banner - Sidebar */}
            <div>
              <AdBanner position="content_sidebar" />
            </div>
