...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const createUserProfile = async (userId: string): Promise<boolean> => {
    ...
    try {
      ...
      // 1. displayName from manual signup (email/password registration)
      // 2. Email username part (works for both manual and Google OAuth)
      // 3. Random username as fallback
      // Note: We skip full_name/name from Google OAuth because they often contain
      // spaces (e.g., "Ahmet Yılmaz") which are not suitable for usernames
      const initialDisplayName = userData.user.user_metadata?.displayName ||
                                userData.user.email?.split('@')[0] ||
                                `user${Math.floor(Math.random() * 10000)}`;

      // Get avatar from Google OAuth if available
  ...
  const sendPasswordResetEmail = async (email: string, languageCode?: string) => {
    ...
    console.log('🔑 PASSWORD RESET EMAIL - DEBUG INFO');
    console.log('   - Email:', email);
    console.log('   - Origin:', window.location.origin);
    console.log('   - Full Redirect URL:', redirectUrl);
    console.log('   - Language Code:', languageCode || 'not provided');
    ...
    console.log('   - Supabase should redirect user to:', redirectUrl);
    console.log('   - Check email for the actual link Supabase sends');
  };
  ...
  return (
    <AuthContext.Provider value={{ user, session, loading, signIn, signInWithGoogle, signUp, signOut, sendPasswordResetEmail, userProfile, fetchUserProfile }}>
      {children}
