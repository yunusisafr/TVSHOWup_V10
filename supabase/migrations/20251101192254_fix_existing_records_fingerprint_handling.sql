/*
  # Fix Existing Records Without Fingerprints

  ## Problem
  Existing user sessions don't have browser_fingerprint values (NULL).
  When users try to use AI chat, the system tries to merge with NULL fingerprints
  causing confusion between old sessions and new fingerprint-based tracking.

  ## Solution
  1. Clean up old guest sessions (older than 24 hours)
  2. Allow fingerprint matching to work with NULL values for backward compatibility
  3. Update check_and_reset function to handle NULL fingerprints gracefully

  ## Changes
  - Delete guest sessions older than 48 hours
  - Update check_and_reset to handle NULL fingerprints
  - Ensure new sessions always get fingerprints
*/

-- Clean up old guest sessions (older than 48 hours) to prevent confusion
DELETE FROM ai_chat_usage_limits
WHERE user_id IS NULL
  AND last_reset_at < (now() - interval '48 hours');

-- Update check_and_reset_ai_chat_limits to handle NULL fingerprints better
...
BEGIN
  ...
  IF p_user_id IS NOT NULL THEN
    ...
    -- If found, update fingerprint if provided
    IF v_record.id IS NOT NULL AND p_browser_fingerprint IS NOT NULL THEN
      UPDATE ai_chat_usage_limits
      SET browser_fingerprint = p_browser_fingerprint, updated_at = v_now
      WHERE id = v_record.id;
      v_record.browser_fingerprint := p_browser_fingerprint;
    END IF;
  -- For guests with fingerprint, find by fingerprint first
  ELSIF p_browser_fingerprint IS NOT NULL THEN
    ...
    IF v_record.id IS NOT NULL AND v_record.session_id != p_session_id THEN
      RAISE NOTICE 'Fingerprint match found - updating session % to existing record', p_session_id;
      UPDATE ai_chat_usage_limits
    ...
    -- If not found by fingerprint, try session_id (backward compatibility)
    IF v_record.id IS NULL THEN
      SELECT * INTO v_record FROM ai_chat_usage_limits WHERE session_id = p_session_id;
      
      -- If found by session_id, add fingerprint to it
      IF v_record.id IS NOT NULL THEN
        RAISE NOTICE 'Session match found - adding fingerprint to existing record';
        UPDATE ai_chat_usage_limits
        SET browser_fingerprint = p_browser_fingerprint, updated_at = v_now
        WHERE id = v_record.id;
        v_record.browser_fingerprint := p_browser_fingerprint;
    ...
    END IF;
  ELSE
    -- Fallback: find by session_id only (no fingerprint provided)
    SELECT * INTO v_record FROM ai_chat_usage_limits WHERE session_id = p_session_id;
  END IF;
...
-- Update increment_ai_chat_usage to handle NULL fingerprints better
...
BEGIN
  ...
  ELSIF p_browser_fingerprint IS NOT NULL THEN
    ...
    -- Fallback to session_id if no fingerprint match
